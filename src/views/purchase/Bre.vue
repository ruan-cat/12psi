    ...
    <div class="layerBtn">
      <el-button-group>
        <template
          v-if="
            form.id == 0 ||
            (form.examine == 0 && $lib.getUserRoot(store.root, 'bre', 'edit'))
          "
        >
...
export default {
        ...
        column: [
          {
            ...
            headerComponentParams: { dispose: { key: "bre" } },
  ...
  created() {
    this.init();
    this.$bus.on("buildBre", this.buildBre);
  },
  methods: {
    //初始化
    init() {
      if (this.source == null) {
        ...
        this.form.number = "CGTHD" + this.$moment().format("YYMMDDHHmmssS");
      } else {
        this.$axios.post("bre/get", { parm: this.source }).then((result) => {
  ...
  beforeDestroy() {
    this.$bus.off("buildBre", this.buildBre);
  },
};
</script>
