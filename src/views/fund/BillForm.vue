    ...
    <div class="layout">
        ...
        <el-form class="searchFrom" ref="searchFrom" inline>
          ...
          <el-form-item>
            <el-select
              ...
              clearable
            >
              <el-option label="预收冲应收" :value="1"></el-option>
              <el-option label="预付冲应付" :value="2"></el-option>
              <el-option label="应收冲应付" :value="3"></el-option>
              <el-option label="销退冲销售" :value="4"></el-option>
              <el-option label="购退冲采购" :value="5"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-date-picker
      ...
      <el-button-group>
        <template v-if="tableSelection.length != 0">
          ...
          <template v-if="$lib.getUserRoot(store.root, 'bill', 'del')">
            <el-button @click="del(tableSelection)" type="info">删除</el-button>
          </template>
        </template>
        <template v-if="$lib.getUserRoot(store.root, 'bill', 'batch')">
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bill.dialog">
          <Bill :source="bill.source" @destroy="bill.dialog = false"></Bill>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
...
<script>
import Bill from "@/views/fund/Bill";
...
export default {
  ...
  data() {
    return {
      ...
      page: {
        ...
        count: 5,
        status: [{ text: "总核销金额", type: "sum", key: "pmy" }],
      },
      bill: {
        dialog: false,
        source: null,
  ...
  methods: {
    ...
    record(page) {
      ...
      this.$axios.post("bill/record", parm).then((result) => {
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "bill/exports?" + parm);
        }, 1000);
      }
    },
    //表格选中数据改变
