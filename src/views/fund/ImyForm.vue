    ...
    <div class="layout">
      ...
      <el-button-group>
        <template v-if="tableSelection.length != 0">
          ...
          <template v-if="$lib.getUserRoot(store.root, 'imy', 'del')">
            <el-button @click="del(tableSelection)" type="info">删除</el-button>
          </template>
        </template>
        <template v-if="$lib.getUserRoot(store.root, 'imy', 'batch')">
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bill.dialog">
          <Imy :source="bill.source" @destroy="bill.dialog = false"></Imy>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
...
<script>
import Imy from "@/views/fund/Imy";
...
export default {
  ...
  methods: {
    ...
    record(page) {
      ...
      this.$axios.post("imy/record", parm).then((result) => {
    ...
    downTemplate() {
      ...
      setTimeout(() => {
        window.open(this.store.base.cdnSite + "/erp/batch/收款单导入模板.xlsx");
      }, 1000);
    },
    //上传模板回调
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "imy/exports?" + parm);
        }, 1000);
      }
    },
    //表格选中数据改变
