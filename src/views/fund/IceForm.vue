    ...
    <div class="layout">
      ...
      <el-button-group>
        <template v-if="tableSelection.length != 0">
          ...
          <template v-if="$lib.getUserRoot(store.root, 'ice', 'del')">
            <el-button @click="del(tableSelection)" type="info">删除</el-button>
          </template>
        </template>
        <template v-if="$lib.getUserRoot(store.root, 'ice', 'batch')">
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bill.dialog">
          <Ice :source="bill.source" @destroy="bill.dialog = false"></Ice>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
...
<script>
import Ice from "@/views/fund/Ice";
...
export default {
  ...
  methods: {
    ...
    record(page) {
      ...
      this.$axios.post("ice/record", parm).then((result) => {
    ...
    downTemplate() {
      ...
      setTimeout(() => {
        window.open(
          this.store.base.cdnSite + "/erp/batch/其它收入单导入模板.xlsx"
        );
      }, 1000);
    },
    //上传模板回调
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "ice/exports?" + parm);
        }, 1000);
      }
    },
    //表格选中数据改变
