        ...
        <el-form class="searchFrom" ref="searchFrom" inline>
          ...
          <el-form-item>
            <el-select
              ...
              clearable
            >
              <el-option label="其它出库单" :value="1"></el-option>
              <el-option label="盘亏单" :value="2"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-select
        ...
        <template v-if="tableSelection.length != 0">
          <template v-if="$lib.getUserRoot(store.root, 'extry', 'examine')">
            ...
            >
              ...
              <ul>
                <template
                  v-if="$lib.getUserRoot(store.root, 'extry', 'examine')"
                >
          ...
          <template v-if="$lib.getUserRoot(store.root, 'extry', 'del')">
            <el-button @click="del(tableSelection)" type="info">删除</el-button>
          </template>
        </template>
        <template v-if="$lib.getUserRoot(store.root, 'extry', 'batch')">
    ...
    >
      <transition name="el-fade-in">
        <template v-if="bill.dialog">
          <Extry :source="bill.source" @destroy="bill.dialog = false"></Extry>
        </template>
      </transition>
    </el-dialog>
    <el-dialog
...
<script>
import Extry from "@/views/room/Extry";
...
export default {
  ...
  methods: {
    ...
    record(page) {
      ...
      this.$axios.post("extry/record", parm).then((result) => {
    ...
    downTemplate() {
      ...
      setTimeout(() => {
        window.open(
          this.store.base.cdnSite + "/erp/batch/其它出库单导入模板.xlsx"
        );
      }, 1000);
    },
    //上传模板回调
    ...
    exports(scene) {
      if (this.tableSelection.length == 0) {
        ...
        setTimeout(() => {
          window.open(this.$base.web + "extry/exports?" + parm);
        }, 1000);
      }
    },
    //表格选中数据改变
