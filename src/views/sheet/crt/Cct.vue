<template>
	<div class="cct area">
		...
		>
			...
			<el-table-column prop="actual" label="应收金额" align="center" width="100px"></el-table-column>
			<el-table-column prop="money" label="实收金额" align="center" width="100px"></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//获取数据
			record() {
				if(this.searchFrom.customer==null){
					this.$message({type: "warning",message: '请选择客户后搜索!'});
				}else{
					let parm = this.searchFrom;
					this.$axios.post("crt/cct", parm).then(result => {
			...
			exports(){
				...
				setTimeout(() => {
					window.open(this.$base.web+'crt/cctExports?'+parm)
				}, 1000);
			},
			//页面刷新
			reload() {
