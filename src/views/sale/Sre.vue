    ...
    <div class="layerBtn">
      <el-button-group>
        <template
          v-if="
            form.id == 0 ||
            (form.examine == 0 && $lib.getUserRoot(store.root, 'sre', 'edit'))
          "
        >
...
export default {
        ...
        column: [
          {
            ...
            headerComponentParams: { dispose: { key: "sre" } },
          ...
          },
          {
            ...
            cellEditor: "serial",
            cellEditorParams: {
              dispose: { record: { show: true, parm: { state: 1 } } },
            },
            hide: false,
            dispose: true,
          },
  ...
  created() {
    this.init();
    this.$bus.on("buildSre", this.buildSre);
  },
  methods: {
    //初始化
    init() {
      if (this.source == null) {
        ...
        this.form.number = "XSTHD" + this.$moment().format("YYMMDDHHmmssS");
      } else {
        this.$axios.post("sre/get", { parm: this.source }).then((result) => {
    ...
    save() {
      ...
      this.$lib
        ...
        .then(() => {
          let effect = this.handleGrid(true);
          if (effect == true) {
  ...
  beforeDestroy() {
    this.$bus.off("buildSre", this.buildSre);
  },
};
</script>
