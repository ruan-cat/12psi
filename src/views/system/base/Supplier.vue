<template>
	<div class="supplier area">
		<div class="layout">
			<el-popover ref="searchPopover" popper-class="searchPopover" placement="bottom-start">
				<el-form class="searchFrom" ref="searchFrom"  inline>
					<el-form-item>
						<el-input placeholder="请输入供应商名称" v-model="searchFrom.name" clearable></el-input>
					</el-form-item>
					<el-form-item>
						<el-input placeholder="请输入供应商编号" v-model="searchFrom.number" clearable></el-input>
					</el-form-item>
					<el-form-item>
						<el-select v-model="searchFrom.category" placeholder="请选择供应商类别" clearable>
							<template v-for="category in store.sys.srCategory">
		...
		<el-table :data="tableData" height="calc(100% - 90px)" @selection-change="selectionChange" border v-madeTable>
			...
			<el-table-column prop="name" label="供应商名称" align="center" width="260px"></el-table-column>
			<el-table-column prop="number" label="供应商编号" align="center" width="180px"></el-table-column>
			<el-table-column prop="category" label="供应商类别" align="center" width="180px"></el-table-column>
			<el-table-column prop="balance" label="应付款余额" align="center" width="120px"></el-table-column>
			<el-table-column prop="frameData.name" label="所属组织" align="center" width="180px"></el-table-column>
			<el-table-column prop="userData.name" label="所属用户" align="center" width="180px"></el-table-column>
						...
						<el-tabs v-model="dialog.active">
							<el-tab-pane label="基础资料" name="base">
								<el-form-item label="供应商名称" prop="name">
									<el-input placeholder="请输入供应商名称" v-model="form.name" clearable></el-input>
								</el-form-item>
								<el-form-item label="供应商编号" prop="number">
									<el-input placeholder="请输入供应商编号" v-model="form.number" clearable></el-input>
								</el-form-item>
								...
								<el-form-item label="供应商类别">
									<el-select v-model="form.category" placeholder="请选择供应商类别">
										<template v-for="category in store.sys.srCategory">
											<el-option :label="category" :value="category"></el-option>
										</template>
									  </el-select>
								</el-form-item>
								<el-form-item label="增值税税率" prop="rate">
									<el-input placeholder="请输入增值税税率" v-model="form.rate" clearable>
							...
							<template v-if="store.fields.hasOwnProperty($route.name)">
								<el-tab-pane label="扩展信息" name="more">
									<FieldForm v-model="form.more" rule="supplier" prefix="more."></FieldForm>
								</el-tab-pane>
							</template>
						</el-tabs>
					</el-form>
					...
					<el-tabs v-model="batch.active">
						<el-tab-pane label="导入数据" name="import">
							...
							<el-row style="text-align:center;">
								...
								<el-col :span="12">
									<el-upload
										:action="$base.web+'supplier/import'"
	...
	export default {
					...
					rate: [
						...
						},
						{
							validator: (rule,value,callback)=>{
								this.$lib.validate('percentage',value)?callback():callback(new Error('增值税税率不正确'));
							},
							trigger: "blur"
						}
					]
		...
		methods: {
			...
			record(page) {
				...
				this.$axios.post("supplier/record", parm).then(result => {
			...
			save() {
				this.$refs["form"].validate(valid => {
					if (valid) {
						this.$axios.post("supplier/save", this.form).then(result => {
							if (result.state == "success") {
								this.record(0);
			...
			downTemplate(){
				...
				setTimeout(() => {
					window.open(this.store.base.cdnSite+"/erp/batch/供应商导入模板.xlsx")
				}, 1000);
			},
			//上传模板回调
			...
			exports(){
				if(this.tableSelection.length==0){
					...
					setTimeout(() => {
						window.open(this.$base.web+'supplier/exports?'+parm)
					}, 1000);
				}
			},
			//表格选中数据改变
