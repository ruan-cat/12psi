<template>
	<div class="account area">
		<div class="layout">
			<el-popover ref="searchPopover" popper-class="searchPopover" placement="bottom-start">
				<el-form class="searchFrom" ref="searchFrom"  inline>
					<el-form-item>
						<el-input placeholder="请输入账户名称" v-model="searchFrom.name" clearable></el-input>
					</el-form-item>
					<el-form-item>
						<el-input placeholder="请输入账户编号" v-model="searchFrom.number	" clearable></el-input>
					</el-form-item>
					<el-form-item>
		...
		<el-table :data="tableData" height="calc(100% - 90px)" border>
			<el-table-column prop="name" label="账户名称" align="center" width="220px"></el-table-column>
			<el-table-column prop="number" label="账户编号" align="center" width="160px"></el-table-column>
			<el-table-column prop="frameData.name" label="所属组织" align="center" width="160px"></el-table-column>
			<el-table-column prop="extension.money" label="账户余额" align="center" width="120px"></el-table-column>
				...
				<template v-if="dialog">
					<el-form :model="form" :rules="rules" ref="form" label-width="80px" >
						<el-form-item label="账户名称" prop = "name">
							<el-input placeholder="请输入账户名称" v-model="form.name" clearable></el-input>
						</el-form-item>
						<el-form-item label="账户编号" prop = "number">
							<el-input placeholder="请输入账户编号" v-model="form.number" clearable></el-input>
						</el-form-item>
						<el-form-item label="所属组织" prop="frame">
							<nodTree v-model="form.frame" :treeData="store.frame" placeholder="请选择所属组织"></nodTree>
						</el-form-item>
						<el-form-item label="余额日期" prop="time">
							<el-date-picker placeholder="请输入余额日期" v-model="form.time" clearable type="date"></el-date-picker>
						</el-form-item>
						<el-form-item label="期初余额" prop="initial">
							<el-input placeholder="请输入期初余额" v-model="form.initial" clearable></el-input>
						</el-form-item>
	...
	export default {
					...
					initial: [
						...
						},
						{
							validator: (rule,value,callback)=>{
								this.$lib.validate('numerical',value)?callback():callback(new Error('期初余额不正确'));
							},
							trigger: "blur"
						}
					]
		...
		methods: {
			...
			record(page) {
				...
				this.$axios.post("account/record", parm).then(result => {
			...
			save() {
				this.$refs["form"].validate(valid => {
					if (valid) {
						this.$axios.post("account/save", this.form).then(result => {
							if (result.state == "success") {
								this.record(0);
								this.dialog = false;
								this.$message({
